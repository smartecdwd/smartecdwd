...
<html lang="en">
  <head>
    ...
    <script type="text/javascript">
        ...
        {
            CKEDITOR.replace( 'text_editor' , {
            filebrowserUploadUrl: "ckeditor/fileupload.php",});	
        });
    </script>
    <script>
      /*CKEDITOR.replace( 'text_editor', {
			...
			<div class="row d-flex align-items-stretch no-gutters">
				<div class="col-md-9 p-4 p-md-5 order-md-last bg-light" style="border:1px solid #1eaaf1;box-shadow: rgba(0, 0, 0, 0.24) 0px 3px 8px;">
                    ...
                    <div class="row" style="margin-top: 10px">
                        <div class="col-12">
                               <form action="iqac_add.php" method="post" enctype="multipart/form-data">
                                    <div class="form-group">
                                        <label class="label" for="name" style="font-size:15px;color:#000;font-weight:bold">About IQAC</label>
...
<?php
    if(isset($_POST['submit']))
    {
        ...
        $qry="insert into iq_about (abt_det) values('$dbj')";
        $res=mysqli_query($conn,$qry);
        $qry="select max(iq_aid) as fid from iq_about";
        $res=mysqli_query($conn,$qry);
        ...
        for($i=0;$i<4;$i++)
        {
            if($_FILES['f1']['size'][$i]>0)
            {
                ...
                $qry="insert into iq_abt_file (iq_aid,fname) values(".$row['fid'].",'$file')";
                $res=mysqli_query($conn,$qry);
            }
        }
