			...
			<div class="row d-flex align-items-stretch no-gutters">
				<div class="col-md-12 p-4 p-md-5 order-md-last bg-light" style="border:1px solid #1eaaf1;box-shadow: rgba(0, 0, 0, 0.24) 0px 3px 8px;">
                    ...
                    <div class="row" style="margin-top: 10px">
                        <div class="col-12">
                            <form action="chan_add.php" method="post" enctype="multipart/form-data">
                                <div class="form-group">
                                    <label class="label" for="name" style="color:#000;font-size:14px">Name</label>
							        <input type="text" name="cn" class="form-control" style="color:#000;font-size:14px">
						        </div>
						        <div class="form-group">
							        <label class="label" for="name" style="color:#000;font-size:14px">Qualification</label>
							        <input type="text" name="qu" class="form-control" style="color:#000;font-size:14px;">
						        </div>
						        <div class="form-group">
							        <label class="label" for="name" style="color:#000;font-size:14px">Details</label>
							        <input type="text" name="cdes" class="form-control" style="color:#000;font-size:14px;">
						        </div>
						         <div class="form-group">
							        <label class="label" for="name" style="color:#000;font-size:14px">Phone</label>
							        <input type="text" name="ph" class="form-control" style="color:#000;font-size:14px;">
						        </div>
						        <div class="form-group">
						        ...
						        <div class="form-group">
							        ...
							        <input type="text" name="mi" class="form-control" style="font-size:14px">
						        </div>
...
<?php
    if(isset($_POST['submit']))
    {
        ...
        $cd=trim($_REQUEST['cdes']);//(str_replace("'",'-', $_REQUEST['cdes']));
        $qu=trim($_REQUEST['qu']);
        $ph=trim($_REQUEST['ph']);
        $mi=trim($_REQUEST['mi']);
        $imgContent="";
        if(($_FILES['f1']['size']>0) && (($_FILES['f1']['type']=="jpg") || ($_FILES['f1']['type']=="jpeg")))
        { 
            $image = $_FILES['f1']['tmp_name']; 
            $imgContent = addslashes(file_get_contents($image)); 
        }	
        $qry="insert into adm_page (name,descp,ch_pic,design,ph,quali,info) values('$cn','$cd','$imgContent','$dg','$ph','$qu','$mi')";
        $res=mysqli_query($conn,$qry);
        
        $qry="select max(slno) as slno from adm_page";
        $res=mysqli_query($conn,$qry);
        ...
        if(($_FILES['f2']['size']>0) &&  ($_FILES['f2']['type']=="pdf"))
        {
            ...
            { 
                ...
                $qry="update adm_page set profile='".$imgContent1."' where slno=".$slno;
                $res=mysqli_query($conn,$qry);
            }
        }
