			...
			<div class="row d-flex align-items-stretch no-gutters">
				<div class="col-md-12 p-4 p-md-5 order-md-last bg-light" style="border:1px solid #1eaaf1;box-shadow: rgba(0, 0, 0, 0.24) 0px 3px 8px;">
                    ...
                    <div class="row" style="margin-top: 10px">
                        <div class="col-12">
                               <form action="dep_app_add.php" method="post" enctype="multipart/form-data">
...
<?php
    if(isset($_POST['submit']))
    {
        ...
        $qry="insert into dep_app (did,app_det) values('$dn','$dbj')";
        $res=mysqli_query($conn,$qry);
        $qry="select max(dalid) as fid from dep_app";
        $res=mysqli_query($conn,$qry);
        $row=mysqli_fetch_array($res);
        for($i=0;$i<4;$i++)
        {
            if($_FILES['f1']['size'][$i]>0)
            {
                ...
                $qry="insert into dep_app_file (dalid,fname) values(".$row['fid'].",'$file')";
                $res=mysqli_query($conn,$qry);
            }
        }
