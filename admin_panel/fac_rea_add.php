      ...
      <div class="container">
        <div class="row no-gutters slider-text align-items-center justify-content-center">
          <div class="col-md-9 ftco-animate text-center">
            <h1 class="mb-2 bread" style="font-family:Exo;color:#fff;font-size:18px;padding:7px"><?php echo $_SESSION['dname']; ?> - Research</h1>
          </div>
        </div>
      </div>
    </section>
		...
		<div class="container">
			<div class="row d-flex align-items-stretch no-gutters">
				<div class="col-md-12 p-4 p-md-5 order-md-last bg-light" style="border:1px solid #1eaaf1;box-shadow: rgba(0, 0, 0, 0.24) 0px 3px 8px;">
                    ...
                    <div class="col-12">
					     <form action="fac_rea_add.php" method="post" enctype="multipart/form-data">
...
<?php
    if(isset($_POST['submit']))
    {
        ...
        if($rt=='Awardees')
            ...
            $rt='Ph.D Ongoing';
        else if ($rt=='Undertaken')
            $rt='Projects Undertaken';
            
        $qry="insert into fac_research (did,rea_det,rea_typ) values('$dd','$dbj','$rt')";
        $res=mysqli_query($conn,$qry);
        $qry="select max(frid) as fid from fac_research";
        $res=mysqli_query($conn,$qry);
        $row=mysqli_fetch_array($res);
        ...
        for($i=0;$i<4;$i++)
        {
            if($_FILES['f1']['size'][$i]>0)
            {
                ...
                $qry="insert into fac_research_file (frid,fname) values(".$row['fid'].",'$file')";
                $res=mysqli_query($conn,$qry);
            }
        }
