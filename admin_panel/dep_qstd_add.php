...
<html lang="en">
  <head>
    <title>Karnatak University | Department - SLET / NET / JRF</title>
    <meta charset="utf-8">
      ...
      <div class="container">
        <div class="row no-gutters slider-text align-items-center justify-content-center">
          <div class="col-md-9 ftco-animate text-center">
            <h1 class="mb-2 bread" style="font-family:Exo;color:#fff;font-size:18px;padding:7px"><?php echo $_SESSION['dname'];?> - SLET / NET / JRF</h1>
			...
			<div class="row d-flex align-items-stretch no-gutters">
				<div class="col-md-12 p-4 p-md-5 order-md-last bg-light" style="border:1px solid #1eaaf1;box-shadow: rgba(0, 0, 0, 0.24) 0px 3px 8px;">
                    ...
                    <div class="row" style="margin-top: 10px">
                        <div class="col-12">
                               <form action="dep_qstd_add.php" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="form-group">
                                        <label class="label" for="name" style="font-size:14px;color:#000">Department SLET / NET / JRF</label>
                                        <textarea id="text_editor"  name="text_editor"></textarea>
						            </div>
						          <!--  <div class="form-group">
                                        <label class="label" for="name" style="font-size:14px;color:#000">Qualified Students </label>
                                        <select name="qty" class="form-control" style="font-size:14px">
                                            <option>SLET</option>
                                            <option>NET</option>
                                            <option>JRF </option>
                                        </select>
...
<?php
    if(isset($_POST['submit']))
    {
        ...
        $qh=$_REQUEST['qty'];
        $qry="insert into dep_qstd (did,qstd_det) values('$dn','$dbj')";
        $res=mysqli_query($conn,$qry);
        $qry="select max(dqid) as fid from dep_qstd";
        $res=mysqli_query($conn,$qry);
        $row=mysqli_fetch_array($res);
        for($i=0;$i<4;$i++)
        {
            if($_FILES['f1']['size'][$i]>0)
            {
                ...
                $qry="insert into dep_qstd_file (dqid,fname) values(".$row['fid'].",'$file')";
                $res=mysqli_query($conn,$qry);
            }
        }
