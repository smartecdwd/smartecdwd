...
<html lang="en">
  <head>
    <title>Karnatak University | Department - Programmes Offered</title>
    <meta charset="utf-8">
	...
	<link href="https://fonts.googleapis.com/css2?family=Exo:wght@500&family=Fredoka+One&family=Signika:wght@500&family=Zen+Kurenaido&display=swap" rel="styles heet">
      ...
      <div class="container">
        <div class="row no-gutters slider-text align-items-center justify-content-center">
          <div class="col-md-9 ftco-animate text-center">
            <h1 class="mb-2 bread" style="font-family:Exo;color:#fff;font-size:18px;padding:7px"><?php echo $_SESSION['dname'];?>  -  Programmes Offered</h1>
          </div>
        </div>
      </div>
    </section>
		...
		<div class="container">
			<div class="row d-flex align-items-stretch no-gutters">
				<div class="col-md-12 p-4 p-md-5 order-md-last bg-light" style="border:1px solid #1eaaf1;box-shadow: rgba(0, 0, 0, 0.24) 0px 3px 8px;">
					...
					<form action="dep_prg_off_add.php" method="post" enctype="multipart/form-data">
						<div class="form-group">
                       	    ...
                       	    ?>
							<input class="form-control" name="dp" id="dp" style="font-size:14px" readonly="true" value='<?php echo $row['dname'];?>'>
			            </div>
						 <div class="form-group">
                            <label class="label" for="name" style="font-size:14px;color:#000">Department Programmes Offered </label>
					...
					<!--	<div class="form-group">
						    <label class="label" for="name" style="font-size:14px;color:#000">Course Name</label>
							<input type="text" class="form-control" style="font-size:14px" name="cn" required>
						</div>
						<div class="form-group">
						    <label class="label" for="name" style="font-size:14px;color:#000">Duration</label>
							<input type="text" class="form-control" style="font-size:14px" name="du" required>
						</div>
						<div class="form-group">
						    <label class="label" for="name" style="font-size:14px;color:#000">Eligibility Criteria</label>
							<textarea rows="5" class="form-control" style="font-size:14px" name="ec" required></textarea>
						</div>-->
...
<?php
     if(isset($_POST['submit']))
    {
       ...
       // $dn=$_REQUEST['dp'];
        ...
        $qry="insert into dep_prg_off (did,prgm) values('$dd','$dbj')";
        $res=mysqli_query($conn,$qry);
        $qry="select max(dpid) as fid from dep_prg_off";
        $res=mysqli_query($conn,$qry);
        $row=mysqli_fetch_array($res);
        ...
        for($i=0;$i<4;$i++)
        {
            if($_FILES['f1']['size'][$i]>0)
            {
                ...
                $qry="insert into dep_prg_files (dpid,fname) values(".$row['fid'].",'$file')";
                $res=mysqli_query($conn,$qry);
            }
        }
        /*if($_FILES['f1']['size']>0)
        {
            $qry="insert into dep_prg_files (dpid,fname) values(".$row['fid'].",'$filename')";
            $res=mysqli_query($conn,$qry);
        }*/
        //echo $qry;
