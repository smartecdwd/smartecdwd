	    ...
	    <div class="container d-flex align-items-center">
	      ...
	      <div class="collapse navbar-collapse" id="ftco-nav">
				<ul class="navbar-nav ml-auto">
					<li class="nav-item active"><a href="news.php" class="nav-link active" style="font-size:18px;color:#000">Home</a></li>
                        ...
                        <div class="col-12">
                               <form action="news_add.php" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="form-group">
            							...
            							<input class="form-control" name="da" id="da" style="font-size:14px" required>
						            </div>
						            <div class="form-group">
                                   	    <label class="label" for="name" style="font-size:14px;color:#000">Status </label>
            							<select class="form-control" name="st" id="st" style="font-size:14px;width:40%">
						            ...
						            <div class="form-group">
            							...
            							<input class="form-control" name="ft" id="ft" style="font-size:14px" required>
						            </div>
                                    <div class="form-group">
                                        <label class="label" for="name" style="font-size:14px;color:#000">File Upload</label>
                                        <input type="file" name="f1" class="form-control" style="font-size:14px" accept="image/jpg,image/jpeg,image/png">
						            </div>
						            
						           	<div class="form-group" style="padding-top:20px;text-align:center">
							            <center><button class="btn btn-danger py-3 px-5"><a href='news.php' style='color:#fff'>Cancel</a></button>
...
<?php
    if(isset($_POST['submit']))
    {
        ...
        $qry="insert into news (ndate,ndata,status) values('$dt','$da','$st')";
        $res=mysqli_query($conn,$qry);
        
        $qry="select max(nwid) as nwid from news";
        $res=mysqli_query($conn,$qry);
        
        $row=mysqli_fetch_array($res);
        
        $allowed_types = array ( 'jpg', 'jpeg', 'png' );
        $ext = pathinfo($_FILES['f1']['name'], PATHINFO_EXTENSION);
        echo "<script> alert('".$_FILES['f1']['size']." --- ". $ext ."'); </script>";
        if(($_FILES['f1']['size']>0) && (($ext=='png') || ($ext=='jpg') || ($ext=='jpeg')))
        {
           ...
           // $ext = pathinfo($file, PATHINFO_EXTENSION);
            ...
            $qry="insert into news_file (nwid,ftitle) values(".$row['nwid'].",'$file')";
            $res=mysqli_query($conn,$qry);
