...
<html lang="en">
  <head>
    <title>Karnatak University | Department - Faculty Publication</title>
    <meta charset="utf-8">
      ...
      <div class="container">
        <div class="row no-gutters slider-text align-items-center justify-content-center">
          <div class="col-md-12 ftco-animate text-center">
            <h1 class="mb-2 bread" style="font-family:Exo;color:#fff;font-size:18px;padding:7px"><?php echo $_SESSION['dname'];?> - Achievements</h1>
          </div>
        </div>
      </div>
    </section>
		...
		<div class="container">
			<div class="row d-flex align-items-stretch no-gutters">
				...
				<div class="row" style="margin-top: 10px">
                        <div class="col-12">
					<form action="fac_ach_add.php" method="post" enctype="multipart/form-data">
			            ...
			            <input type="text" name="tdd" value='<?php echo $_SESSION['did']; ?>' style='visibility:hidden'>
			             <input type="text" name="par" value='<?php echo $pa; ?>' style='visibility:hidden'>
					</form>
				</div>
...
<?php
    if(isset($_POST['submit']))
    {
        ...
        $qry="insert into fac_ach (did,ah_det,ah_type) values('$dn','$dbj','$ah')";
        $res=mysqli_query($conn,$qry);
       // echo $qry;
        $qry="select max(fhid) as fid from fac_ach";
        $res=mysqli_query($conn,$qry);
        $row=mysqli_fetch_array($res);
        $id=$row['fid'];
       for($i=0;$i<4;$i++)
        {
            if($_FILES['f1']['size'][$i]>0)
            {
                ...
                $qry="insert into fac_ach_file (fhid,fname) values(".$id.",'".$file."')";
                $res=mysqli_query($conn,$qry);
               // echo $qry;
