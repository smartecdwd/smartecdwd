			...
			<div class="row d-flex align-items-stretch no-gutters">
				...
				<div class="row" style="margin-top: 10px">
                    <div class="col-12">
                        <?php
                            $nid=$_REQUEST['nid'];
                            $qry="select * from notification where nid=".$nid." order by ndate desc";
                            $res=mysqli_query($conn,$qry);
                            $fl="";
                            while($row=mysqli_fetch_array($res))
                            {
                                echo "<h4>".$row["nname"]."</h4>";
                                echo $row['ndesc'];
                            }
                        ...
                        <table id="grid" class="table table-bordered" style="font-size:11pt;color:#000;width:80%" align="center">
                            <tr style="background-color:#1eaaf1;color:#fff;text-align:center">
                                <td>Slno</td>
                                <td style="width:70%">Notification Name </td>
                                <td>View</td>
                            </tr>
                            <?php
                                //$nid=$_REQUEST['nid'];
                                $qry="select * from notification_file where nid=".$nid." order by nfid";
                                $res=mysqli_query($conn,$qry);
                                $cnt=1;
                                while($row=mysqli_fetch_array($res))
                                {
                                    ...
                                    echo "<td style='text-align:center'><a href='noti/".$row['fname']."' target='_blank'><img src='images/v2.jpg'> </a></td>";
                                    echo "</tr>";
